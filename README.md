...
Live in ~1 minute at: **https://www.useplenty.in/**

> The repo must be **public** for free GitHub Pages. `.nojekyll` skips the Jekyll build.

## Waitlist form (FormSubmit)

The email form POSTs to `https://formsubmit.co/ajax/sharvilabs@gmail.com` — a free,
...
`sharvilabs@gmail.com`. Click the link in it, otherwise submissions are not delivered.
...
- Custom domain: `CNAME` pins GitHub Pages to `www.useplenty.in`.
- Privacy policy (required for Play listing): add `privacy.html` here and link it
  from the footer → `https://www.useplenty.in/privacy.html`.
